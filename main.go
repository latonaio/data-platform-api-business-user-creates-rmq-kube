...
import (
	dpfm_api_caller "data-platform-api-busienss-user-creates-rmq-kube/DPFM_API_Caller"
	dpfm_api_input_reader "data-platform-api-busienss-user-creates-rmq-kube/DPFM_API_Input_Reader"
	"data-platform-api-busienss-user-creates-rmq-kube/config"
	"encoding/json"
	"fmt"
	"time"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	errs := caller.AsyncBusinessUserCreates(accepter, &input, l)
	if len(errs) != 0 {
